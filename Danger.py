...
# Replace MongoDB with JSON offline storage
OFFLINE_STORAGE_FILE = 'users.json'

# Function to load users from the JSON file
def load_users():
    if os.path.exists(OFFLINE_STORAGE_FILE):
        with open(OFFLINE_STORAGE_FILE, 'r') as file:
            return json.load(file)
    return {}

# Function to save users to the JSON file
def save_users(users):
    with open(OFFLINE_STORAGE_FILE, 'w') as file:
