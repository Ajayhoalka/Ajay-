...
TOKEN = '7842166624:AAHgEMncOH2u-ynhlxiObsQplYwjRQT02lU'
...
def handle_attack_command(message):
   ...
   # Dictionary to keep track of ongoing attacks by their chat_id
ongoing_attacks = {}

def start_attack(chat_id, target_ip, target_port, duration):
    # Check if the bot is already processing an attack for this chat_id
    ...
    bot.send_message(chat_id, "*💣 Ready to launch an attack!*\n"
                               "*The attack has been launched on {0}:{1} for {2} seconds.*\n"
                               "*Let the chaos begin! 🎉*".format(target_ip, target_port, duration))

def check_attack_status(chat_id):
    # If the user requests the status of the attack
    if chat_id in ongoing_attacks:
        attack_info = ongoing_attacks[chat_id]
        elapsed_time = time.time() - attack_info['start_time']
        remaining_time = attack_info['duration'] - elapsed_time
        
        if remaining_time <= 0:
            # Attack is finished, remove from ongoing attacks
            del ongoing_attacks[chat_id]
            bot.send_message(chat_id, "*✅ Attack finished!*\n"
                                       "The attack has successfully completed.")
        else:
            bot.send_message(chat_id, "*🕒 Remaining time: {0} seconds*".format(int(remaining_time)))
    else:
        bot.send_message(chat_id, "*⚠️ No attack in progress.*")

def attack(target_ip, target_port, duration):
    # Logic for your attack (you can modify this to your needs)
    pass

# When a new attack command is received:
def handle_attack_command(chat_id, command_args):
    # Assume the command_args are in the format [target_ip, target_port, duration]
    if len(command_args) == 3:
        target_ip = command_args[0]
        target_port = command_args[1]
        duration = int(command_args[2])

        start_attack(chat_id, target_ip, target_port, duration)
    else:
        bot.send_message(chat_id, "*⚠️ Invalid format!*\n"
...
def canary_command(message):
    ...
    button1 = types.InlineKeyboardButton(
        ...
        url="https://t.me/drabfeedback/266")
    button2 = types.InlineKeyboardButton(
